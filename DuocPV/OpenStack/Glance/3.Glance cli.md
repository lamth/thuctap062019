# Thao tác sử dụng với GLance # 
## 1. Sử dụng GLance CLI ## 
### 1.1. Hiển thị danh sách image ### 
- Sử dụng câu lệnh sau để lấy danh sách image được phép truy cập 
```
glance image-list
```
```
...
+--------------------------------------+--------+
```
### 1.2 Hiển thị thông tin chi tiết một image ### 
```
...
```
### 1.3.tạo image ### 
- Để upload image lên thư mục của GLance từ file image có sẵn, sử dụng lệnh `glance image-created` 
```
...
- Một image upload thành công sẽ ở trạng thái active, người dùng có thể đưa nó về trạng thái deactive cũng như thay đổi qua lại giữa hai trạng thái bằng câu lệnh `glance image-deactivate image_id` và `glance image-reactivate image_id`
```
[root@controller ~]# glance image-show a61d6e0f-bf74-4f87-b410-eec94e193e70
+------------------+----------------------------------------------------------------------------------+
| Property         | Value                                                                            |
...
```
- Thu được token, ta gán token thu được vào biến sủ dụng 

### 3.2.Lấy danh sách imgae ### 
```
...
```
### 3.3. Xem thông tin chi tiết imgae ### 
